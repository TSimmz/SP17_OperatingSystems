  ...
  while (1)
    {
	  ...
	  if (!strcmp(command, "exit"))
	  ...
	  else if (!strcmp(command, "start"))
		{
		  ...
		  {
			  sem_post(&wiz);
			  fprintf(stderr,"S COMMAND HERE\n");
			  //step-by-stop semaphore
			  //sem_wait(mutex);
		  }
...
main(int argc, char** argv)
{
      ...
      for (j = 0; j < cube_size; j++)
		{
		  ...
		  /* Fill in ******************************************************/
		  //TODO: initialize room semaphores for lock/unlock
  ...
  for (i = 0; i < teamA_size; i++)
	...
	//  pthread_create(&widA[i], NULL, wizard_func, &wizard_descr);
    }

  /* Team B */
  //pthread_t widB[teamB_size];
  for (i = 0; i < teamB_size; i++)
	...
	//  pthread_create(&widB[i], NULL, wizard_func, &wizard_descr);
    }

  /* Fill in **********************************************/
  // Initialize the wizard/user semaphores
  // wait so user gets control first
  sem_init(&user, 0, 1);
  sem_init(&wiz, 0, 0);
